...
shift = createUsherShift({ scene, world, camera, collisionWorld, storage: null, seed: 23,
  ...
  onStart: event => { media.onStart(event); patrons?.onStart(event); }, onBreak: event => patrons?.onBreak(event) });
...
add("sign-hall", "Hall signs · sheet/watch layering", [-22.6, 1.68, 57.8], [-17.8, 2.8, 55.2]);
...
document.querySelector("#watch").onclick = () => shift.toggleWatch();
...
function testEvent(kind) {
  ...
  return { id: `review-${++syntheticEvent}-${kind}`, theaterId: room.id, number: room.number, kind, time: shift.schedule.minute };
}
document.querySelector("#start-show").onclick = () => {
  audio.start(); media.prepare(); media.retry(); paused = false;
  document.querySelector("#pause").textContent = "Pause";
  const event = testEvent("start"); shift.doors.onStart(event.theaterId); media.onStart(event); patrons.onStart(event);
...
document.querySelector("#break-show").onclick = () => {
  ...
  shift.cleaning.beginBreak(event.theaterId, `review-break-${syntheticEvent}`); patrons.onBreak(event); refreshViews();
};
async function post(endpoint, body) {
  const response = await fetch(`http://127.0.0.1:5183/${endpoint}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
...
document.querySelector("#shot").onclick = async () => {
  try { await shot(`${views[+select.value].id}${shift.sheet.visible ? "-sheet" : shift.watch.visible ? "-watch" : shift.placementActive ? "-place" : "-active"}`); status.textContent = "View captured"; }
  catch (error) { status.textContent = String(error); }
};
document.querySelector("#capture").onclick = async event => {
  event.target.disabled = true; capturing = true; action = false; shift.sheet.hide(); shift.watch.hide();
  try {
    ...
    shift.update(0, { active: true }); shift.toggleWatch(); shift.watch.update(0); await shot("sign-hall-watch"); shift.watch.hide();
    await post("index", { views, snapshot: shift.getSnapshot(), media: media.getSnapshot(), patrons: patrons.getSnapshot() });
    status.textContent = `Capture complete · ${views.length + 2} views`;
  } catch (error) { status.textContent = String(error); }
...
renderer.setAnimationLoop(() => {
  ...
  shift.update(dt, { active, action }); patrons.update(dt, active); media.update(dt, active);

...
export function createShowCustomers({ scene, world, collisionWorld, camera, doors, waste }) {
  ...
  let scheduleEvents = null, scheduledStarts = [], startCursor = 0, simulationRate = 1;
  ...
  let frontExit = []; const startedShows = new Set(), usedSeats = new Map();
  ...
  for (const room of AUDITORIUMS) for (let index = 0; index < MAX_SHOW_AUDIENCE; index++) {
    ...
    actors.push({ group, fallback, fallbackLimbs: limbs, limbs, packaging, collider, room: room.id, index, state: "idle", path: [], waypoint: 1,
      walked: 0, time: 0, speed: (1.10 + index % 4 * .035) * 1.4, waiting: false, departing: false, seat: null, disposal: null, retry: 0,
  ...
  function noteSeated(actor) {
    if (!usedSeats.has(actor.show)) usedSeats.set(actor.show, new Set());
    usedSeats.get(actor.show).add(actor.seat.id);
  }
  function planExit(actor) {
    ...
    if (completeExit.length && origin.distanceTo(completeExit[0]) < .2) {
      setPath(actor, completeExit, "exit"); return;
    }
    if (actor.state === "planning") return;
    actor.state = "planning"; actor.collider.enabled = true;
    pathJob(async () => {
      const path = await navigation.pathAsync(origin, PATRON_EXIT);
      if (disposed || actor.state !== "planning") return;
      if (path) setPath(actor, path, "exit");
      else { actor.state = "leaving"; actor.retry = 2; }
    }).catch(error => { console.warn("Customer exit route unavailable", error); actor.state = "leaving"; actor.retry = 5; });
  }
  function prepareMemberRoute(member, room) {
    if (member.route || member.pending || clock < (member.retryAt ?? 0)) return;
    member.pending = true;
    pathJob(async () => {
      member.route = (await navigation.theaterSeatAsync(room, member.seat.id))?.route;
      if (member.route) routes.set(room, true);
      else member.retryAt = clock + 10;
    }).catch(error => { member.retryAt = clock + 10; console.warn("Customer seat route unavailable", error); })
      .finally(() => { member.pending = false; });
  }
  function tryPassingToss(actor, dt) {
    actor.tossRetry = Math.max(0, (actor.tossRetry ?? 0) - dt);
    actor.tossGesture = Math.max(0, (actor.tossGesture ?? 0) - dt);
    if (!actor.departing || !actor.packaging.visible || actor.tossRetry > 0) return;
    actor.tossRetry = .12;
    const from = actor.group.position.clone().setY(actor.group.position.y + 1.3);
    const target = waste?.getPassingDisposalTarget?.(from, actor.collider.id);
    if (target && waste.throwCustomerTrash({ binId: target.binId, from,
      units: 7 + actor.index % 4 * 2, shape: actor.index % 3, sourceColliderId: actor.collider.id })) {
      actor.packaging.visible = false; actor.tossGesture = .48; stats.tossed++;
    }
  }
  function enqueue(event, kind, scheduled = false) {
    ...
    const version = event?.attendanceVersion ?? activeShow?.attendanceVersion ?? 26;
    const showKey = room + ":" + cycle + ":" + version;
    let indices = null;
    if (kind === "start") { startedShows.add(showKey); if (!usedSeats.has(showKey)) usedSeats.set(showKey, new Set()); }
    else if (startedShows.has(showKey)) {
      // Fast-forward must not resurrect patrons whose arrival window expired.
      indices = roomActors.get(room).filter(actor => actor.show === showKey && actor.state !== "idle").map(actor => actor.index);
      for (let i = queue.length - 1; i >= 0; i--) if (queue[i].kind === "start" && queue[i].room === room && queue[i].cycle === cycle) queue.splice(i, 1);
    }
    queue.push({ key, room, kind, cycle, version, indices, time: clock, startMinute: scheduled && Number.isFinite(event?.time) ? event.time : null }); return true;
  }
  ...
  function audience(room, cycle, version = 26) {
    const key = `${room}:${cycle}:${version}`;
    if (!audiences.has(key)) {
      const plan = roomPlans.get(room), attendance = createShowAttendance(plan, { cycle, version });
      ...
      audiences.set(key, { ...attendance, members, key, version });
    }
  ...
  function assign(actor, member, show) {
    ...
    actor.show = show.key; actor.showCycle = show.cycle; actor.attendanceVersion = show.version; actor.tossRetry = actor.tossGesture = 0; actor.packaging.visible = false; actor.stalled = 0; actor.repathAt = 0; actor.motion.set(0, 0, 0);
  }
  function serviceQueue() {
    ...
    for (const event of queue) {
      ...
      const show = audience(event.room, event.cycle, event.version);
      ...
      event.done ??= new Set();
      for (const index of event.indices ?? show.members.map((_, index) => index)) {
        ...
        if (event.kind === "start") {
          ...
          if (!member.route) { prepareMemberRoute(member, event.room); continue; }
          ...
          if (actor.state === "idle") { assign(actor, member, show); poseSeated(actor); actor.state = "seated"; noteSeated(actor); }
          if (actor.state === "seated") {
            // Keep waiting patrons fully seated until the route is ready and
            // the person ahead has cleared their standing space.
            if (!member.route) { prepareMemberRoute(member, event.room); continue; }
            actor.baseRoute = member.route;
            const stand = new THREE.Vector3(...actor.seat.stand);
            if (actors.some(other => other !== actor && other.collider.enabled
              && Math.abs(other.group.position.y - stand.y) < 1.6 && Math.hypot(other.group.position.x - stand.x, other.group.position.z - stand.z) < .9)) continue;
            leaveSeat(actor);
          }
    ...
    for (let i = queue.length - 1; i >= 0; i--) if ((queue[i].done?.size ?? 0) === (queue[i].indices?.length ?? audience(queue[i].room, queue[i].cycle, queue[i].version).count)) queue.splice(i, 1);
  }
  ...
  function walk(actor, dt) {
    ...
    if (following && !actor.departing && !inRoom && actor.party && distance > 1.5 && navigation.clearance(p.x, p.z, p.y) > 1) {
    ...
    if (!actor.departing && !inRoom && actor.party && navigation.clearance(p.x, p.z, p.y) > .85) {
    ...
    // Departures keep their place in a single-file lane. Stop behind the
    // person ahead instead of orbiting around their capsule at a bottleneck.
    if (actor.departing) {
      const leader = movingBodies.find(other => {
        if (other === actor || !other.departing || !other.collider.enabled || Math.abs(other.group.position.y - p.y) > .4) return false;
        // A strict exit-progress order breaks circular yielding where rows
        // merge. Without this, two different waypoint headings can each see
        // the other person as "ahead" and wait forever.
        const progress = other.exitRemaining - actor.exitRemaining;
        if (progress > 0 || progress === 0 && other.rank > actor.rank) return false;
        if (other.stalled > 2) return false;
        ...
        return along > .04 && along < .88 && side < .42 && goal
          && direction.x * (goal.x - other.group.position.x) + direction.z * (goal.z - other.group.position.z) > 0;
      });
      if (leader) {
        actor.waiting = true; actor.motion.set(0, 0, 0); actor.stalled += dt;
        actor.limbs.forEach(({ arm, leg }) => { arm.rotation.x = leg.rotation.x = 0; });
    ...
    for (const angle of [0, -.35, .35, -.75, .75, -1.15, 1.15, -1.57, 1.57, -2.15, 2.15, Math.PI]) {
      ...
      // This step is at most 9 cm, well below the capsule radius: endpoint
    ...
    if (actor.stalled > 1.1 && !closedDoor && (playerNear || crossing || actor.stalled > 5)) replan(actor);
  ...
  function step(dt) {
    ...
    for (const actor of movingBodies) {
      let remaining = 0, from = actor.group.position;
      for (let i = actor.waypoint; i < actor.path.length; i++) { remaining += from.distanceTo(actor.path[i]); from = actor.path[i]; }
      actor.exitRemaining = remaining;
    }
    serviceQueue();
    for (const actor of actors) {
      ...
      if (actor.state === "sitting") {
        ...
        if (actor.time >= .8) { actor.state = "seated"; noteSeated(actor); stats.entered++; } continue;
      }
      if (actor.state === "standing") {
        ...
        if (actor.time >= .65 && actors.some(other => other !== actor && other.collider.enabled
          ...
          actor.time = 0; poseSeated(actor); continue;
        }
      ...
      if (actor.tossGesture > 0) actor.limbs[1].arm.rotation.x = -1.2 * Math.sin(actor.tossGesture / .48 * Math.PI);
    }
  }
  return {
    root, actors, navigation,
    getUsedSeatIds(event) {
      if (!enabled) return undefined;
      const room = idOf(event), active = roomActors.get(room)?.find(actor => actor.state !== "idle");
      const cycle = Number.isInteger(event?.cycle) || Number.isInteger(event?.audienceCycle) ? attendanceCycle(event, event?.kind ?? "break") : active?.showCycle;
      const version = event?.attendanceVersion ?? active?.attendanceVersion ?? 26;
      const seats = usedSeats.get(`${room}:${cycle}:${version}`);
      return seats ? [...seats] : undefined;
    },
    onStart(event) { return enqueue(event, "start", scheduleMinute !== null); }, onBreak(event) { return enqueue(event, "break"); },
    syncSchedule({ minute, events = [], done = [], timeScale = 2 } = {}) {
      ...
      scheduleMinute = minute; simulationRate = THREE.MathUtils.clamp(timeScale / 5, 1, 4);
    ...
    restoreSchedule({ minute = 0, done = [], events = [], mode } = {}) {
      ...
      for (const room of AUDITORIUMS) {
        ...
        const cycle = start ? attendanceCycle(start, "start") : 0;
        const version = start?.attendanceVersion ?? roomEvents[0]?.attendanceVersion ?? (mode === "legacy" ? 24 : 26);
        const show = audience(room.id, cycle, version);
        let pending;
        if (start) {
        ...
        show.members.forEach((member, index) => {
          ...
          actor.state = "seated"; noteSeated(actor); actor.departing = false; actor.group.visible = false; restored++; pending?.done.add(index);
        });
      }
      for (let i = queue.length - 1; i >= 0; i--) if ((queue[i].done?.size ?? 0) === (queue[i].indices?.length ?? audience(queue[i].room, queue[i].cycle, queue[i].version).count)) queue.splice(i, 1);
    ...
    update(delta, active = true, schedule = null) {
      ...
      carry += Math.min(.1, Math.max(0, delta)) * simulationRate;

...
export function createUsherGameplay({ scene, world, camera, collisionWorld, showToast = () => {}, onSound = () => {}, storage,
  ...
  const visuals = createCleaningVisuals({ scene, world }), { root, tools, stored } = visuals;
  const ownCollider = collisionWorld?.addBox({ id: "usher-cart", minX: 23.71, maxX: 24.89, minZ: 54.38, maxZ: 55.02, minY: 0, maxY: .94 });
  const look = new THREE.Vector3(), forward = new THREE.Vector3(), right = new THREE.Vector3(), ray = new THREE.Ray();
  const box = new THREE.Box3(), toolBounds = new Map();
  for (const object of [...Object.values(tools), ...Object.values(stored)]) {
    object.updateWorldMatrix(true, true); toolBounds.set(object, new THREE.Box3().setFromObject(object).applyMatrix4(object.matrixWorld.clone().invert()));
  }
  ...
  function fullToolClear(object, ignoreId = null) {
    object.updateWorldMatrix(true, false); box.copy(toolBounds.get(object)).applyMatrix4(object.matrixWorld);
    return !(collisionWorld?.colliders ?? []).some(c => c.enabled !== false && c.id !== ignoreId
      && box.max.x > c.minX + .002 && box.min.x < c.maxX - .002 && box.max.y > c.minY + .002
      && box.min.y < c.maxY - .002 && box.max.z > c.minZ + .002 && box.min.z < c.maxZ - .002);
  }
  ...
  function findSurface() {
    ...
    for (const entry of visuals.surfaces.values()) {
      const s = entry.surface, seat = s.seatId && entry.job.seats.find(seat => seat.id === s.seatId);
  ...
  function findFloor() {
    if (look.y >= -.18) return null;
    const feet = camera.position.y - 1.68; let y = feet, p;
    for (let i = 0; i < 4; i++) {
      const distance = (y - camera.position.y) / look.y;
      if (distance <= 0 || distance > 3) return null;
      p = ray.at(distance, new THREE.Vector3()); y = world.groundHeight(p.x, p.z, feet);
    }
    p.y = y;
    const job = state.jobs.find(j => { const b = plans.find(p => p.id === j.id).bounds; return p.x > b.xMin && p.x < b.xMax && p.z > b.zMin && p.z < b.zMax; });
    if (!job || Math.hypot(p.x - camera.position.x, p.z - camera.position.z) > 1.8
  ...
  function refreshFocus() {
    ...
    const kitPoint = new THREE.Vector3(24.3, 1.38, 54.4);
    if (camera.position.distanceTo(kitPoint) < 2.3 && ray.distanceToPoint(kitPoint) < .6 && clearSegment(camera.position, kitPoint, "usher-cart")) focus = { kind: "kit" };
  ...
  function selectTool(kind) {
    ...
    if (!state.kit) { showToast("Take the cleaning kit from the usher cart first."); return false; }
  ...
  function interact() {
    ...
    if (focus.kind === "kit") { state.kit = true; selectTool(state.heldTool === "broom" ? "cloth" : "broom"); showToast("Kit ready. 1 broom · 2 cloth · Q holster."); }
    else if (focus.kind === "bin") {
  ...
  function positionTools(action, dt) {
    ...
    if (state.pouring) {
      ...
      visible.pan = fullToolClear(tools.pan, liveBin?.ignoreColliderId ?? state.pourTarget.ignoreColliderId);
      visible.broom = fullToolClear(tools.broom);
    ...
    if (!target || !state.heldTool || (target.kind === "tray" && state.heldTool === "broom")) {
      ...
      for (const [name, object] of Object.entries(tools)) {
        ...
        visible[name] = clearSegment(camera.position, object.position) && fullToolClear(object);
      }
      return {};
    }
    ...
    visible.broom = clearAt(head, 2.1) && fullToolClear(tools.broom, chair ? ignore : null);
    visible.pan = clearAt(panPoint, 2.1) && fullToolClear(tools.pan);
    visible.cloth = clearAt(at, 1.95) && fullToolClear(tools.cloth, ignore);
    const contact = { canMove };
    if (state.heldTool === "broom" && visible.broom && (chair || visible.pan)) {
    ...
    } else if (state.heldTool === "cloth" && action && visible.cloth && target.surface) {
  ...
  return {
    ...
    get focusDistance() {
      if (focus?.kind === "kit") return camera.position.distanceTo(new THREE.Vector3(24.3, 1.38, 54.4));
    ...
    get focusedPrompt() {
      ...
      if (focus?.kind === "kit") return state.kit ? "Swap cleaning tool at cart" : "Take portable cleaning kit";
    ...
    get hint() {
      if (!state.kit) return "Take the portable cleaning kit from the usher cart.";
      ...
      if (target?.seat) return `Seat ${target.seat.label}: ${seatStage(target.job, target.seat)}. Hold and move to clean.`;
    ...
    getSnapshot() {
      return { status: this.status, heldTool: state.heldTool, kit: state.kit, handsOwner: hands.owner, summary: usherSummary(state),
        ...
        contactPose, anchors: { cart: [24.3, 1.4, 54.4], bins: getBinTargets(),
    ...
    dispose() { if (disposed) return; disposed = true; save(); if (hands.owner === "cleaning") hands.owner = null;
      if (ownCollider) collisionWorld?.remove(ownCollider); visuals.dispose(); },
  };
}

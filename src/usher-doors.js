...
import { AUDITORIUMS, COURTYARD_PLAN } from "./layout-data.js";
...
export function createUsherDoors({ scene, camera, collisionWorld, showToast = () => {}, storage }) {
  ...
  const doors = AUDITORIUMS.map(room => {
    ...
    const width = court?.width ?? (room.number === 6 ? 2.2 : 2.05);
    const height = room.number === 6 ? 2.18 : 2.48;
    const x = planToWorldX(room.entry.center);
    const z = room.entry.outerPlaneZ ?? (room.screenSide === "north" ? room.bounds.zMin : room.bounds.zMax);
    const inward = room.screenSide === "north" ? 1 : -1;
    const previous = saved?.[room.id];
    const open = typeof previous?.open === "boolean" ? previous.open : true;
    const door = { id: room.id, number: room.number, x, z, width, height, inward,
    ...
    for (const sign of [-1, 1]) {
      ...
      hinge.position.set(x + sign * (width / 2 - .15), 0, z); root.add(hinge);
      const leafWidth = width / 2 - .175, direction = -sign;
      ...
      box("panel", [direction * leafWidth / 2, height / 2, 0], [leafWidth, height - .10, .045], panel);
  ...
  function boxesAt(door, leaf, angle) {
    const theta = -door.inward * leaf.direction * angle, cos = Math.cos(theta), sin = Math.sin(theta);
    ...
    return leaf.colliders.map((_, i) => {
      ...
      const x = leaf.hinge.position.x + lx * cos, z = door.z - lx * sin;
      return { minX: x - hx, maxX: x + hx, minZ: z - hz, maxZ: z + hz, minY: .05, maxY: door.height - .05 };
    });
  }
  function apply(door) {
    for (const leaf of door.leaves) {
      leaf.hinge.rotation.y = -door.inward * leaf.direction * door.angle;
  ...
  return {
    ...
    getSnapshot() { return doors.map(d => ({ id: d.id, angle: d.angle, targetOpen: d.targetOpen, startDue: d.startDue, center: [d.x, 1, d.z], handles: d.leaves.map(l => l.hinge.localToWorld(new THREE.Vector3(l.direction * l.leafWidth * .65, 1.05, 0)).toArray()) })); },
    dispose() { save(); root.removeFromParent(); colliders.forEach(c => collisionWorld.remove(c)); geometry.dispose(); panel.dispose(); metal.dispose(); },
  };
}

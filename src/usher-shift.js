...
import { createUsherWatch } from "./usher-watch.js";
...
export function createUsherShift(options) {
  ...
  const watch = createUsherWatch({ scene, camera, schedule });
  ...
  function chooseFocus() {
    if (sheet.visible || watch.visible) return null;
  ...
  return {
    cleaning, waste, supplies, schedule, doors, sheet, watch, hands,
    update(delta, input = {}) {
      if (disposed) return;
      active = Boolean(input.active);
      if (active && !schedule.started) schedule.begin();
      schedule.update(delta, active);
      const reading = sheet.visible || watch.visible;
      ...
      sheet.update();
      watch.update(active ? delta : 0);
    },
    interact() {
      if (!active) return false;
      if (sheet.visible) { sheet.hide(); return true; }
      if (watch.visible) { watch.hide(); return true; }
    ...
    selectTool(kind) {
      if (!active) return false;
      cancelAction = true; sheet.hide(); watch.hide(); return cleaning.selectTool(kind);
    },
    toggleSheet() { if (active) { cancelAction = true; watch.hide(); modules[hands.owner]?.cancelPlacement?.(); return sheet.toggle(); } return false; },
    toggleWatch() { if (active) { cancelAction = true; sheet.hide(); modules[hands.owner]?.cancelPlacement?.(); return watch.toggle(); } return false; },
    togglePlacement() { if (!active) return false; cancelAction = true; sheet.hide(); watch.hide(); return modules[hands.owner]?.togglePlacement?.() ?? false; },
    ...
    get canPlace() { return active && !sheet.visible && !watch.visible && Boolean(modules[hands.owner]?.canPlace); },
    get heldTool() { return sheet.visible || watch.visible ? null : modules[hands.owner]?.heldTool ?? null; },
    get actionLabel() { const owner = modules[hands.owner]; return owner?.actionLabel ?? (owner?.heldTool === "cloth" ? "HOLD TO WIPE" : "HOLD TO SWEEP"); },
    get focusedPrompt() { return sheet.visible ? "Fold break sheet" : watch.visible ? "Lower watch" : selected?.focusedPrompt ?? ""; },
    get hint() {
      if (sheet.visible) return "Bold rows start: close doors. Regular rows break: clean the theater. B folds the sheet.";
      if (watch.visible) return `${schedule.time} · One real minute = two shift minutes · T lowers your watch`;
    ...
    getSnapshot() { return { schedule: schedule.getSnapshot(), cleaning: cleaning.getSnapshot(), waste: waste.getSnapshot(), supplies: supplies.getSnapshot(), doors: doors.getSnapshot(), heldTool: this.heldTool, sheetVisible: sheet.visible, watchVisible: watch.visible, placementActive: this.placementActive, hands: { ...hands } }; },
    dispose() { if (disposed) return; disposed = true; sheet.dispose(); watch.dispose(); Object.values(modules).forEach(m => m.dispose()); doors.dispose(); schedule.dispose(); },
  };
}

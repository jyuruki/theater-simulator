...
| Counters | `counter_blue`, `counter_white`, `counter_wood` | Finished face, stainless plinth and stone top, fitted per existing straight counter segment. |
...
`node scripts/smoke-integrated-assets.mjs` also loads the actual library into the complete theater world. It checks 2,467 placements using 215 shared-material batches, all 1,093 recliners and 1,175 shared armrests, every one of the 82 seating rows against actual row colliders and neighboring meshes, all 32 used model families (the spare stanchion has no existing location), guest-facing candy/water display orientation, unchanged collision data, fitted footprints, retained fallback ownership, and disposal of imported and furnishing resources.

...
- `+Y` is elevation.
- The player begins outside the center of the translated main-lobby entrance. V16 temporarily interpreted the façade as three double-depth vestibule banks; V17 replaces that inferred second plane with the photographed single glass frontage. The current façade reads from physical left to right as two large waist-height-sill windows, one structural divider pillar, then six double-door assemblies grouped `2 · gap · 2 · gap · 2` on one plane. The approximately two-foot group gaps are fixed glass panels. V14 retained the concession/office side's established plan-X translation and compact box-office/stair edge while the ticket approach, theater hall, and auditorium modules remained fixed. V15 then translated only the complete Theater 1/2 pair by `+1 m` in plan X toward the ticket podium; both footprints, their shared wall, cubbies, doors, bowls, seating, and internal routes move as one rigid module.
...
- The public stone lobby alone uses a `10.8 m` exposed volume; its lowest pipework sits roughly one to two feet above the `8.9 m` mural/menu-screen datum. V16 preserves every established pipe and duct route in plan, compresses only their elevation beneath the lower roof, and adds matching runs across the previously bare stair/kiosk half. The red-carpet corridors, dark-tile fountain court, manager office, overflow/snack room, kitchen storage, and kitchen service areas stay at the established 4.6-meter height. V14 runs the diagonal mural structure from the kitchen-door side to the isolated back-bar start. Its botanical artwork keeps the established dimensions, while equal gray panels occupy only the left and right ends—never above or below it. A continuous clipped soffit closes the entire underside without projecting into the kitchen roof. V17 mounts four compact customer kiosks flush to the east wall and places three kiosk-width showtime screens above them; one POS on the long box-office leg, one central wooden lectern, and two ceiling-height fountain-island pillars remain source-authored fixtures rather than renderer-only guesses.
- V15 mounts a `7.3 × 4.3 m` digital menu screen on the north wall above the bar. Its four original menu/food compositions rotate on a deterministic 10-second interval. This is a display fixture, not a change to the established bar footprint.
- The stair/kiosk side carries a second original mural based on the supplied opposite-lobby view. Its foliage-left, warm-portrait-right composition remains deliberately distinct from the concession botanical mural across the lobby. V16 adds equal gray fields only beside this second artwork—never above or below it—and does not move or reshape the V14 concession mural surround, clipped soffit, kitchen roof, two-thirds service wall, connector nook, or sealed floor wedge.
...
- V16 pulls the east perimeter wall directly onto the 2-meter stair edge and removes the former side gap. The first thin tread begins at the end of the box-office return; 38 open-underneath treads climb to an `8 m`-high, `2 × 2 m` square landing and closed upper door. A white wall lower than the murals follows the lobby-facing flight edge and turns 90 degrees beneath the landing, producing an accessible nook rather than a solid stair support. The outer perimeter is the east guard, while a railing continues along the open lobby side; the flight occupies the landing's full south seam, so no cross-rail blocks the final tread.
- `BB` and `GB` are represented as men's and women's restrooms using the individual drawings rather than a shared template. BB keeps its left-left privacy path, nine south-wall stalls, six north-wall urinals, one long north-wall sink, and wide clear fixture aisle. V15 recesses the complete nine-stall bank into the south wall without changing its count, sequence, or the rest of the bathroom layout. Its exterior is split into a distinct vertical drinking-fountain nook and the separate recessed `MEN` cubby immediately beside it. V10 makes that cubby 50% wider and moves the complete BB/water-fountain/nook/trash frontage only a few inches away from Theater 3, creating the intended small jamb reveal without rearranging the cluster. BB's finished back wall meets Theater 3's lower-storage/service side as one shared boundary rather than two overlapping shells. GB keeps fourteen stalls and three separate north-wall sinks, with its two six-stall banks aligned on the same partition grid. Fixtures are wall-relative so mirrors, sinks, and partitions remain inside their rooms.

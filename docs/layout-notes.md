...
- `+Y` is elevation.
- The player begins outside the main lobby entrance at approximately plan `(1.5, 0, -9.3)` after the V9 front-block translation.
...
- The entrance leads into a large gray-stone lobby/service block, then a long maroon-carpeted ticket approach, then the east–west theater hall. In V9 the long Theater 9-side run is 6.7 meters wide; it steps down to the previous 4.2-meter width exactly at the wall carrying the two drinking fountains and remains that width through the Theater 2 exit. The complete front/south block moved rigidly 2.5 meters to create that space, with its internal room proportions unchanged.

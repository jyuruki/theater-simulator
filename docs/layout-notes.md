...
- `+Y` is elevation.
- The player begins outside the translated main-lobby entrance. V13 retains the concession/office side's established plan-X translation while tightening the box-office/stair edge; the ticket approach, theater hall, and auditorium modules remain fixed.
...
- `KS`, `K`, `B`, `C`, and `OFF` are Kitchen Storage, Kitchen, Bar, Concession, and Office. V13 retains the complete front service block's rigid translation and the guest-bar alignment with the fixed ticket-approach wall. The five traced counter vertices remain intact: the horizontal guest bar stays wood, while the concession face reads white service → blue register/candy → white Expo. The blue run carries the explicit sequence `2 POS · candy · 2 POS · candy · 2 POS`. The taller paired poppers sit against the revised counter-parallel service wall. The original kitchen boundary now seals the triangular remnant created by the V12 wall change as inaccessible dead space; its roof closes at the service datum, while the office-side wall rises to the mural top and creates the requested attic-like volume above the working kitchen. The first office door enters an interim candy-overflow room; a second door reaches the manager office.
- The public stone lobby alone uses the high exposed volume at three times the standard corridor height. The red-carpet corridors, dark-tile fountain court, manager office, overflow/snack room, kitchen storage, and kitchen service areas stay at the established 4.6-meter height. The concession mural's projecting fascia is physically tied to the back wall by a closed lower soffit rather than floating in the high lobby. V13 extends that diagonal surround from beside the kitchen door to just before the back bar and pushes it deeper toward the service wall; the V12 artwork size remains fixed, with gray end panels occupying the new width. Three customer kiosks, one POS on the long box-office leg, one central wooden lectern, and two ceiling-height fountain-island pillars are source-authored fixtures rather than renderer-only guesses.

...
- `BB` and `GB` are represented as men's and women's restrooms using the individual V6 drawings rather than a shared template. BB is reached through the compact Trash–fountain–entry nook and has a left-left privacy path, nine south-wall stalls, six north-wall urinals, one long north-wall sink, and a wider clear fixture aisle. GB forces the drawn south approach, west turn through the red door, then north turn into the main room; it has fourteen stalls and three separate north-wall sinks. Fixtures are wall-relative so mirrors, sinks, and partitions remain inside their rooms.
...
- Theater 3's courtyard door sits roughly five feet from the future-task door; a short stem/dogleg preserves its bowl and lower-storage alignment, and a waist-high divider runs between those doors from the back wall to the fountain island. Theater 6 now begins across from the Theater 13/14 pair beside a closed future-upstairs door, then uses a few feet of straight vestibule, a right turn into a flat transverse hall, and a continuously low-roofed under-tier side passage to the front apron.

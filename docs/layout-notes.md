...
- `+Y` is elevation.
- The player begins outside the middle bank of the translated main-lobby entrance. V16 authors three compact vestibule banks at the physical-right frontage, each with aligned outer and inner double doors; one structural pillar and exactly two large fixed windows continue toward physical-left. V14 retained the concession/office side's established plan-X translation and compact box-office/stair edge while the ticket approach, theater hall, and auditorium modules remained fixed. V15 then translated only the complete Theater 1/2 pair by `+1 m` in plan X toward the ticket podium; both footprints, their shared wall, cubbies, doors, bowls, seating, and internal routes move as one rigid module.

...
- `+Y` is elevation.
- The player begins outside the main lobby entrance at approximately plan `(1.5, 0, -9.3)` after the V9 front-block translation retained by V10.
...
- The V10 minimap reads the compressed two-piece stepped hall, reordered auditorium/service modules, drinking-fountain transition, widened `MEN` cubby, BB footprint, Theater 6 vestibule stair, Theater 3 route and lower-storage rectangles, courtyard west edge, and Theater 9 cubby directly from `src/layout-data.js`. It deliberately has no second coordinate table, so later proportional corrections remain synchronized with the first-person world.

...
- `BB` and `GB` are represented as men's and women's restrooms using the individual drawings rather than a shared template. BB keeps its left-left privacy path, nine south-wall stalls, six north-wall urinals, one long north-wall sink, and wide clear fixture aisle; its exterior now matches the photo with two fountains on a flat wall and a separate recessed `MEN` cubby immediately beside them. GB keeps fourteen stalls and three separate north-wall sinks, with its two six-stall banks aligned on the same partition grid. Fixtures are wall-relative so mirrors, sinks, and partitions remain inside their rooms.

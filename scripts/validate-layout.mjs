...
function assertBoundsNear(actual, expected, message, epsilon = 1e-6) {
  for (const key of ["xMin", "xMax", "zMin", "zMax"]) {
    assertNear(actual[key], expected[key], `${message}.${key}`, epsilon);
  }
}
...
assert.ok(pointInRect(LOBBY_PLAN.kiosks[0].position[0], LOBBY_PLAN.kiosks[0].position[2], LOBBY_PLAN.envelope));
...
assert.deepEqual(
  ...
  [
    ...
    { x: -22.03333333333333, z: 17.5 }, { x: -23.26666666666667, z: 14.3 }, { x: -24.5, z: 11.1 },
    { x: -24.5, z: 9.6 }, { x: -24.5, z: 7 },
  ].map(({ x, z }) => ({ x: x + LOBBY_SHIFT_X, z: z + FRONT_SHIFT_Z })),
  "The V12 kitchen partition must preserve its angled return and two-thirds parallel concession wall.",
);
assert.deepEqual(LOBBY_PLAN.serviceDoor, {
  x: -24.5 + LOBBY_SHIFT_X,
  z: 10.35 + FRONT_SHIFT_Z,
  partitionSegment: 5,
});
...
assert.equal(LOBBY_PLAN.barScreen.wallSide, "north");
assertNear(LOBBY_PLAN.barScreen.wallZ, 21.5, "Bar-screen wall plane");
assertNear(LOBBY_PLAN.barScreen.xMin, -7.8, "Bar-screen west edge");
assertNear(LOBBY_PLAN.barScreen.xMax, -0.5, "Bar-screen east edge");
assertNear(LOBBY_PLAN.barScreen.width, 7.3, "Bar-screen support width");
assertNear(LOBBY_PLAN.barScreen.height, 4.3, "Bar-screen height");
assertNear(LOBBY_PLAN.barScreen.bottomY, 4.6, "Bar-screen bottom");
assertNear(LOBBY_PLAN.barScreen.topY, 8.9, "Bar-screen top");
assertNear(LOBBY_PLAN.barScreen.intervalSeconds, 10, "Bar-screen rotation interval");
assert.deepEqual(LOBBY_PLAN.barScreen.slideIds, ["island-grill", "garlic-fries-feature", "fries-and-rings", "previews-and-morning"]);
assert.equal(LOBBY_PLAN.barScreen.xMin, LOBBY_PLAN.backBar.xMin, "Bar display must begin with the isolated bar wall.");
assert.equal(LOBBY_PLAN.barScreen.xMax, TICKET_APPROACH_PLAN.bounds.xMin, "Bar display must end at the ticket-hall opening.");
assert.equal(LOBBY_PLAN.barScreen.height, LOBBY_PLAN.muralFacade.artwork.height, "Bar screen and concession mural must have equal heights.");
assertUnique(LOBBY_PLAN.barScreen.slideIds, "Bar-screen slide IDs");
...
assertNear(LOBBY_PLAN.oppositeLobbyMural.bottomY, 4.6, "Opposite-lobby mural bottom");
assertNear(LOBBY_PLAN.oppositeLobbyMural.topY, 8.9, "Opposite-lobby mural top");
...
const shiftedLobbyEquipment = new Map([
  ["concession-popper-1", [-22.58, 14.5]], ["concession-popper-2", [-23.16, 13]],
...
assert.deepEqual(
  ...
  "The diagonal run must read POS, POS, candy, POS, POS, candy, POS, POS.",
);
assert.deepEqual(
  CONCESSION_SERVICE_SEQUENCE.map(({ slotIndex }) => slotIndex),
  [0, 1, 2, 3, 4, 5, 6, 7],
  "Concession service slots must retain their guest-facing order.",
);
assert.deepEqual(POS_STATIONS.map(({ id }) => id), Array.from({ length: 6 }, (_, index) => `concession-pos-${index + 1}`));
assert.deepEqual(CONCESSION_CANDY_DISPLAYS.map(({ id }) => id), ["concession-candy-1", "concession-candy-2"]);
for (const [index, item] of CONCESSION_SERVICE_SEQUENCE.entries()) {
  assertNear(item.slotT, (index + 0.5) / 8, `${item.id} service-run fraction`);
  assertNear(
    item.position[0],
    LOBBY_PLAN.concessionRun.start.x
      + (LOBBY_PLAN.concessionRun.end.x - LOBBY_PLAN.concessionRun.start.x) * item.slotT,
    `${item.id} concession-run X`,
  );
  assertNear(
    item.position[2],
    LOBBY_PLAN.concessionRun.start.z
      + (LOBBY_PLAN.concessionRun.end.z - LOBBY_PLAN.concessionRun.start.z) * item.slotT,
    `${item.id} concession-run Z`,
  );
  assertNear(
    item.rotation,
    item.type === "candy"
      ? LOBBY_PLAN.concessionRun.fixtureRotation
      : LOBBY_PLAN.concessionRun.rotation,
    `${item.id} concession-run rotation`,
  );
}

assert.deepEqual(
  LOBBY_PLAN.customerCounterSections.map(({ id, segmentIndex, role, baseMaterialKey, topMaterialKey }) => ({
    id, segmentIndex, role, baseMaterialKey, topMaterialKey,
  })),
  [
...
const concessionParallelStart = LOBBY_PLAN.kitchenPartition[3];
const concessionParallelMidpoint = LOBBY_PLAN.kitchenPartition[4];
const concessionParallelEnd = LOBBY_PLAN.kitchenPartition[5];
const concessionRunDx = LOBBY_PLAN.concessionRun.end.x - LOBBY_PLAN.concessionRun.start.x;
const concessionRunDz = LOBBY_PLAN.concessionRun.end.z - LOBBY_PLAN.concessionRun.start.z;
const parallelDx = concessionParallelEnd.x - concessionParallelStart.x;
const parallelDz = concessionParallelEnd.z - concessionParallelStart.z;
assertNear(parallelDx * concessionRunDz - parallelDz * concessionRunDx, 0, "Concession back-wall/run cross product", 1e-8);
assert.ok(parallelDx * concessionRunDx + parallelDz * concessionRunDz > 0, "Concession back wall must face the same run direction.");
assertNear(Math.hypot(parallelDx, parallelDz), LOBBY_PLAN.concessionRun.length * 2 / 3, "Parallel back-wall length");
assertNear(concessionParallelMidpoint.x, (concessionParallelStart.x + concessionParallelEnd.x) / 2, "Parallel back-wall midpoint X");
assertNear(concessionParallelMidpoint.z, (concessionParallelStart.z + concessionParallelEnd.z) / 2, "Parallel back-wall midpoint Z");
...
assert.deepEqual(muralFacade.grayFills[1].end, muralFacade.surround.end);
assert.deepEqual(muralFacade.returnAnchors, {
...
for (const popper of EQUIPMENT_ANCHORS.filter(({ type }) => type === "popper")) {
  ...
  assertNear(popper.height, 2.8, `${popper.id} full height`);
  assertNear(popper.glassBottom, 1.05, `${popper.id} glass bottom`);
  assertNear(popper.glassTop, 2.48, `${popper.id} glass top`);
  assertNear(popper.canopyBottom, popper.glassTop, `${popper.id} glass/canopy seam`);
  assertNear(popper.canopyTop, 2.76, `${popper.id} canopy top`);
  assert.ok(popper.canopyTop > 2.7, `${popper.id} must read as a tall backline popper.`);
}
...
assert.equal(boysStallBank.side, "south");
assert.equal(boysStallBank.recessedIntoWall, true, "V15 boys stalls must be pushed into a dedicated south-wall recess.");
assertBoundsNear(boysStallBank.recessBounds, {
  xMin: -21.17, xMax: -11.87, zMin: 63.55, zMax: 64.7,
}, "V15 boys stall recess");
assertNear(boysStallBank.recessBounds.zMax, boysMain.zMin, "Boys stall recess/main-room seam");
assertNear(boundsDepth(boysStallBank.recessBounds), boysStallBank.depth, "Boys stall recess depth");
assertNear(boundsWidth(boysStallBank.recessBounds), boysStallBank.end - boysStallBank.start, "Boys stall recess width");
assert.ok(boysStallBank.recessBounds.zMin < boysMain.zMin, "Recessed stalls must occupy wall depth instead of stealing the main aisle.");
const boysStallDoorZ = boysStallBank.recessBounds.zMax - 0.025;
const boysUrinalFrontZ = boysMain.zMax - 0.32 - 0.42 / 2;
assert.ok(boysUrinalFrontZ - boysStallDoorZ >= 2.9, "Recessing the stalls must preserve almost three metres between stall doors and urinals.");
...
assert.deepEqual(FOUNTAIN_PLAN.island, { xMin: -0.5, xMax: 12.1, zMin: 63.48, zMax: 64.9 });
assertNear(FOUNTAIN_PLAN.rearPassage, 2.4, "Fountain island/rear-counter working aisle");
assert.equal(COURTYARD_PLAN.waistPartition.zMin, FOUNTAIN_PLAN.island.zMin, "The waist partition must shift with the island's front edge.");

assert.deepEqual(FOUNTAIN_PLAN.pillars, [
  {
    id: "fountain-island-west-pillar",
    position: [-2.03, 0, FOUNTAIN_PLAN.centerZ],
    footprint: [0.7, 0.7],
    height: LOBBY_CEILING_PLAN.baseHeight,
    finish: "white",
  },
  {
    ...
    position: [13.63, 0, FOUNTAIN_PLAN.centerZ],
    footprint: [0.7, 0.7],
    height: LOBBY_CEILING_PLAN.baseHeight,
    finish: "white",
  },
...
assert.ok(
  ...
  "Every soda/ICEE fixture must move with the island rather than overhang its old location.",
);
